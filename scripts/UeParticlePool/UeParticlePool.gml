...
function UeParticlePool(maxCount) constructor {
    ...
    // ===== FREE LIST APPROACH =====
    // Invece di swap, usiamo un array di indici attivi
    ...
    /**
     * Alloca un nuovo slot particella dalla free list
     */
    ...
    /**
     * Libera uno slot particella (molto più veloce dello swap!)
     */
    static free = function (activeIndex) {
        gml_pragma("forceinline");

        // Prendi l'indice reale dalla active list
        var realIdx = self.activeIndices[activeIndex];

        // Rimetti nella free list
        self.freeIndices[self.freeCount++] = realIdx;

        // Rimuovi dalla active list (swap solo nell'array degli indici!)
        self.aliveCount--;
        self.activeIndices[activeIndex] = self.activeIndices[self.aliveCount];
    }

    /**
     * Ottieni l'indice reale di una particella attiva
     */
